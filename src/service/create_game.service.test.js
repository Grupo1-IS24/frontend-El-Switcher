...
import { createGame } from './create_game.service';
...
describe('createGame', () => {
  ...
  it('debería mostrar una alerta si minPlayers es mayor que maxPlayers', async () => {
    ...
    expect(alert).toHaveBeenCalledWith('El mínimo de jugadores no puede ser mayor al máximo de jugadores');
  });

  it('debería mostrar una alerta si minPlayers es menor que 2', async () => {
    await createGame('game', 'owner', 1, 4);
    expect(alert).toHaveBeenCalledWith('El mínimo de jugadores debe ser al menos 2');
  });

  it('debería mostrar una alerta si maxPlayers es mayor que 4', async () => {
    await createGame('game', 'owner', 2, 5);
    expect(alert).toHaveBeenCalledWith('El máximo de jugadores debe ser como máximo 4');
  });
